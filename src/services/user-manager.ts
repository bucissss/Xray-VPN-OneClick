...
export class UserManager {
      ...
      if (inbound.settings?.clients) {
        for (const client of inbound.settings.clients) {
          users.push({
            ...
            flow: client.flow,
            createdAt: new Date().toISOString(), // TODO: Get from metadata
            status: 'active', // TODO: Get from config
          });
        }
      }
    }
  ...
  async getShareInfo(userId: string): Promise<UserShareInfo> {
    ...
    const serverAddress = 'your-server-ip'; // TODO: Get from config or system
